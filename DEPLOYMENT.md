...
### If everything has to go on Vercel

The container hosts above generally want a card. If that is the blocker, the
whole application can go on Vercel — with one addition and one loss.

**The addition: something has to drive the dispatcher.** The loop that sends
queued email and fires medication reminders runs once a minute inside a live
process, and a serverless function is frozen between requests, so it never gets
a second tick. Nothing looks broken — every request the browser makes still
works — and no reminder ever arrives.

`POST /api/internal/dispatch` runs one pass on demand. Set `DISPATCH_SECRET`,
then point a free external scheduler at it every minute — **cron-job.org** does
minute intervals and asks for no card:

```
POST https://<your-app>/api/internal/dispatch
Header:  X-Dispatch-Secret: <the value of DISPATCH_SECRET>
```

Overlapping calls are safe: the queue is claimed with `FOR UPDATE SKIP LOCKED`,
so two passes take disjoint batches and nothing is sent twice. The response is
the counts — `{"medication": 2, "sent": 3, "pushed": 4}` — so the scheduler's own
history shows whether anything is actually being delivered, not merely that the
endpoint answered.

**The loss: the live vitals stream.** Server-sent events need a connection held
open, which a serverless function will not do. It degrades rather than breaks —
the client sets the feed to "offline" and the page still works from ordinary
requests; a new alert just does not appear on its own.

**The gain, worth stating:** both halves end up on one origin, so the
`SameSite=Lax` cookie problem below disappears entirely.

Installing the PWA on a phone changes none of this. The PWA is the frontend; a
push notification is sent *by the server* to Apple's or Google's push service,
which delivers it to the device. If the server never sends one, an installed app
receives nothing.

### Why the API is better off a container
...
not sent on a cross-*site* request — and `medinova.vercel.app` and
`medinova-api.onrender.com` are different sites. Deployed that way, every
...
```
app.medinova.pk     → Vercel      (CLIENT_ORIGIN)
api.medinova.pk     → the API     (NEXT_PUBLIC_API_URL = https://api.medinova.pk/api)
```
...
```bash
docker build -t medinova-api ./api
docker run -p 4000:4000 --env-file .env medinova-api
```

...
/**
 * Building request URLs, absolute base or relative.
 *
 * `NEXT_PUBLIC_API_URL` is usually a separate host. It is a *relative* path in
 * the one deployment shape where the API and the pages share an origin — and
 * that shape is what removes the `SameSite` question, the shared registrable
 * domain and CORS all at once. So the relative case is worth pinning: it used
 * to throw `TypeError: Invalid URL`, which made that whole configuration
 * unreachable rather than merely untested.
 */
describe("the API base", () => {
  async function urlFor(base: string, path: string, query?: Record<string, unknown>) {
    vi.resetModules();
    ...
    await api.apiRequest(path, { query: query as never });
    vi.unstubAllEnvs();
    return seen[0];
  }

  it("is used as given when it is absolute", async () => {
    expect(await urlFor("https://api.example.org/api", "/auth/me")).toBe(
      "https://api.example.org/api/auth/me",
    );
  });

  it("resolves against the page's origin when it is relative", async () => {
    expect(await urlFor("/api", "/auth/me")).toBe(`${window.location.origin}/api/auth/me`);
  });

  it("keeps the query string either way", async () => {
    expect(await urlFor("/api", "/invoices", { limit: 25, status: "PAID" })).toBe(
      `${window.location.origin}/api/invoices?limit=25&status=PAID`,
    );
  });

  it("still drops empty parameters against a relative base", async () => {
    expect(await urlFor("/api", "/doctors", { search: "", limit: 10 })).toBe(
      `${window.location.origin}/api/doctors?limit=10`,
    );
  });
});

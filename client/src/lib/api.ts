/**
 * Typed client for the MediNova API.
 *
...
/**
 * The base a relative `API_URL` is resolved against.
 *
 * `NEXT_PUBLIC_API_URL` is usually absolute — a separate API host — and then
 * this is unused. It matters for the deployment where the API is served from
 * the *same* origin as the pages, under `/api`: that is the configuration where
 * the session cookie is host-only and simply works, with no `SameSite`
 * question and no shared registrable domain to arrange.
 *
 * `new URL("/api/auth/login")` throws without a base, so leaving this out made
 * the same-origin setup impossible to configure — the app would not start
 * rather than fall back.
 *
 * The server-side value is a placeholder that is never fetched from: every
 * caller of this module runs in the browser.
 */

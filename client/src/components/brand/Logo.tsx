...
/**
 * The MediNova AI mark, drawn from the logo.
 *
 * A rounded medical cross carrying a white ECG pulse, with circuit traces and
 * node dots leaving to the right; the wordmark sets "Medi" in deep
 * blue, "Nova" in the mid blue the gradient passes through, and "AI" in the
 * teal the traces end in. Vector rather than the PNG so it stays crisp at every size,
 * recolours for a dark ground, and costs no request.
 *
 * If `public/brand/MediNova AI_logo.png` is ever added to the repo, this is the
  ...
  return (
    ...
    <span className={cx("font-display font-bold tracking-tight whitespace-nowrap", className)}>
      {/* Three parts, because the mark has three. "Medi" is the deep blue of
          ...
          reads as the same object as the symbol beside it rather than as a
          label pasted next to one.

          A thin space before "AI" rather than a normal one: it is a separate
          word in the name and needs to breathe, but a full space next to a
          two-letter suffix reads as a gap. */}
      <span className={onDark ? "text-white" : "text-[#0B3FA8] dark:text-[#8FB6FF]"}>Medi</span>
      <span className={onDark ? "text-[#7DD3FC]" : "text-[#1A8FC7] dark:text-[#5FD3E8]"}>Nova</span>
      <span className={onDark ? "text-[#5EEAD4]" : "text-[#0FA9B9] dark:text-[#14C4C1]"}>
        {" "}AI
      </span>
    </span>
  );

...
const BUNDLE = {
  format: "medinova.patient-export",
  formatVersion: 1,
  exportedAt: "2026-09-03T09:30:00Z",
  source: { system: "MediNova AI", timezone: "Asia/Karachi" },
...
describe("before anything is pressed", () => {
  ...
  it("offers the printable view as a link, not a button", () => {
    mount();
    // A link so middle-click and "open in new tab" both work — it goes
    // somewhere rather than doing something.
    const link = screen.getByRole("link", { name: /view and print/i });
    expect(link).toHaveAttribute("href", "/patient/export");
  });

  it("puts the PDF first and the raw data second", () => {
    mount();
    // The order is the recommendation. A PDF is what a person means by "my
    // record"; the JSON is for another system and is useless to a human being,
    // so offering the two at equal weight would make the common case a choice
    // nobody has the context to make.
    const pdf = screen.getByRole("button", { name: /download my record/i });
    const json = screen.getByRole("button", { name: /raw data/i });
    expect(pdf.compareDocumentPosition(json) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  });
});

describe("pressing download", () => {
  it("saves a PDF, not just a request", async () => {
    vi.spyOn(patients, "exportRecord").mockResolvedValue(BUNDLE);
    mount();
    await userEvent.click(downloadButton());

    await waitFor(() => expect(clicked).toBe(1));
    expect(anchor.download).toMatch(/^medinova-record-MRN-000123-\d{4}-\d{2}-\d{2}\.pdf$/);
    expect(anchor.href).toContain("blob:record");
  });

  it("the file really is a PDF, drawn rather than screenshotted", async () => {
    vi.spyOn(patients, "exportRecord").mockResolvedValue(BUNDLE);
    mount();
    await userEvent.click(downloadButton());
    ...
    // Rendering the page to a canvas would also produce a valid PDF — a picture
    // of one, with no selectable text, four times the size and blurry in print.
    // The magic number says it is a PDF; the size says something was drawn.
    const head = await blob.slice(0, 5).text();
    expect(head).toBe("%PDF-");
    expect(blob.size).toBeGreaterThan(1000);
  });

  it("still offers the raw bundle as JSON, for another system", async () => {
    vi.spyOn(patients, "exportRecord").mockResolvedValue(BUNDLE);
    mount();

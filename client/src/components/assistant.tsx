    ...
    <div className="space-y-3">
      ...
      <p role="status" aria-live="polite" className="text-sm text-muted">
        {listening
          ...
          : tr(
              "Your speech is turned into text on this device. The recording is never sent to MediNova AI.",
              "Aap ki awaaz isi device par likhai mein badalti hai. Recording kabhi MediNova AI ko nahi bheji jaati.",
            )}
      </p>
...
function ConsentGate({ status, onGranted }: { status: AssistantStatus; onGranted: () => void }) {
  ...
  const points: [string, string, string][] = [
    ...
    [
      "mic",
      "If you use the microphone, your browser turns your speech into text on your device and MediNova AI never receives the recording. Most browsers use their own online service to do that, so the audio reaches the browser's provider rather than ours. You can always type instead.",
      "Microphone istemal karein to aap ka browser awaaz ko isi device par likhai mein badalta hai — MediNova AI ko recording kabhi nahi milti. Aksar browsers iske liye apni online service istemal karte hain, is liye audio browser ke provider tak jaati hai, hum tak nahi. Aap hamesha likh bhi sakte hain.",
    ],
    [
      "add_photo_alternate",
        ...
        <div className="flex items-center gap-3 border-b border-line/70 bg-card/70 px-3 py-2 backdrop-blur-xl sm:px-5">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-display text-sm font-bold text-strong">
              {mode.kind === "symptoms"
                ? tr("Symptom check", "Takleef ka jaiza")
                : (active?.title ?? tr("MediNova AI Assistant", "MediNova AI Assistant"))}
            </p>

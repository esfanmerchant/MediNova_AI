...
could let anyone back in, removing a doctor with money still in flight, and
removing a doctor patients are still booked to see. That last one is the
distinction the module turns on twice: a past appointment is history and is
kept, a future one is a promise and has to be dealt with by a person.
"""
...
REMOVED_EMAIL_DOMAIN = "removed.medinova.invalid"
...
async def _plan_doctor(db: AsyncSession, plan: RemovalPlan, doctor: Doctor) -> None:
    ...
    if upcoming:
        # Appointments are kept above, because they are the patient's history.
        # A *future* one is not history — it is a promise, and removing the
        # only person who can keep it leaves patients holding a confirmed slot
        # with nobody at the other end. They are not told, because nothing here
        # tells them.
        #
        # A blocker rather than a silent cancellation: cancelling somebody's
        # medical appointment is a decision with a patient on the other side of
        # it, and it should be made deliberately by the administrator, who can
        # reassign instead.
        plan.blockers.append(
            f"{upcoming} upcoming appointment(s) are still booked with this doctor. "

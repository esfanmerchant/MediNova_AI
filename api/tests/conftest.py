...
#: Whether a demo population exists to test against.
#:
#: These tests do not create the people they sign in as — they assume a cast
#: (Priya, Vikram, Meera and the doctors who treat them) is already in the
#: database, with a history of appointments and records to read. That cast was
#: removed from this deployment through the administrator's own remove-account
#: screen, which is the feature working rather than an accident, and the
#: removal is permanent by design.
#:
#: So the tier is off unless somebody says they have a population. Skipping is
#: the honest answer: an empty database is not a broken endpoint, and 357
#: failures that all say "invalid credentials" describe the fixtures, not the
#: code under test. Anyone who does have demo accounts — with the emails and
#: password above — sets TEST_DEMO_DATA=1 and the tier runs as it always did.
HAS_DEMO_DATA = os.environ.get("TEST_DEMO_DATA", "").strip() not in ("", "0", "false")

requires_demo_data = pytest.mark.skipif(
    not HAS_DEMO_DATA,
    reason=(
        "no demo population in this database — set TEST_DEMO_DATA=1 if you have "
        "loaded the demo accounts (see README, 'Running the integration tier')"
    ),
)

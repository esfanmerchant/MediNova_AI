...
class Counter:
    ...
    async def execute(self, statement: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        # Appointments are counted twice with different meanings — every one the
        # doctor ever saw, and only the ones still ahead of them — so keying on
        # the table alone would hand both the same answer and no test could tell
        # the time filter from a missing one.
        if name == "appointments" and "startTime" in str(statement.whereclause):
...
class TestWhatItRefuses:
    ...
    async def test_a_doctor_patients_are_still_booked_to_see_is_refused(self) -> None:
        """A confirmed slot with nobody at the other end.

        The appointments themselves are kept either way — they are the patient's
        history. But a future one is not history, and removing the only person
        who can attend it leaves patients holding an appointment that will never
        happen, with nothing in the system to tell them so.
        """
        doctor = Doctor(id="d1", user_id="u1", specialization="Cardiology", license_number="PMC-1")
        plan = await plan_for(
            a_user(Role.DOCTOR),
            counts={"appointments": 9, "appointments_upcoming": 3, "users": 1},
            rows={"doctors": doctor},
        )
        assert not plan.allowed
        assert any("3 upcoming appointment(s)" in b for b in plan.blockers)

    async def test_a_doctor_whose_appointments_are_all_behind_them_may_go(self) -> None:
        """The other half, and the reason the time filter has to be real.

        Nine appointments, none of them ahead: nobody is expecting this doctor.
        A blocker that counted every appointment a doctor ever saw would make
        any experienced doctor permanently unremovable.
        """
        doctor = Doctor(id="d1", user_id="u1", specialization="Cardiology", license_number="PMC-1")
        plan = await plan_for(
            a_user(Role.DOCTOR),
            counts={"appointments": 9, "appointments_upcoming": 0, "users": 1},
            rows={"doctors": doctor},
        )
        assert plan.allowed
        # Still emptied rather than deleted: those nine are somebody's history.
        assert plan.mode == "ANONYMISE"
        assert plan.keeps["appointmentsSeen"] == 9

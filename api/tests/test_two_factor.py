...
class TestEnrolment:
    def test_the_uri_names_the_issuer_and_the_account(self) -> None:
        """Parsed rather than searched for as a substring.

        The issuer now contains a space, which the otpauth URI percent-encodes —
        so a plain `"MediNova AI" in uri` looks for something that is correctly
        never there. Decoding is what the authenticator app does, and asserting
        the decoded value is what actually says the app will show the right name.
        """
        ...
        # The label is "<issuer>:<account>", and both halves are encoded.
        label = unquote(parsed.path.lstrip("/"))
        issuer, _, account = label.partition(":")

        assert issuer == twofactor.TOTP_ISSUER
        assert query["issuer"] == [twofactor.TOTP_ISSUER]
        assert account == "priya@example.com"

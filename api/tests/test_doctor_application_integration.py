...
The reviewer is whichever administrator the environment names, and signing in is
the only thing done with that account — nothing is written to it beyond the
notification rows the flow produces, and those are removed again. Every other
person here is created by these tests and deleted afterwards, taking their
application, documents and sessions with them, so the file needs no demo
population and runs against an empty database.

...
class TestWhatIsRefused:
    ...
    def test_a_patient_cannot_remove_anybody(
        ...
        client.cookies.clear()
        # The fixture's own patient, not a demo account. What is being asserted
        # is that holding the patient role is not enough, which is true of any
        # patient — so borrowing a demo one only tied this check to a
        # population that happens to exist, and it stopped running the day that
        # population was removed.
        client.post("/api/auth/login", json={"email": PATIENT_EMAIL, "password": PASSWORD})

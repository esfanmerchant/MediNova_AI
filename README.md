# MediNova AI — Smart Healthcare Management System

[![CI](https://github.com/esfanmerchant/MediNova_AI/actions/workflows/ci.yml/badge.svg)](https://github.com/esfanmerchant/MediNova_AI/actions/workflows/ci.yml)
...
| `SMTP_FROM` | The visible sender, e.g. `MediNova AI <no-reply@medinova.pk>` |
...
**There are none.** The fictional cast this project carried — Priya, Vikram,
Meera and the doctors who treated them — was removed through the
administrator's own **Remove account** screen. That is the feature working
rather than an accident, and removal is permanent by design, so they are not
coming back.

The running system never depended on them. What did is the integration tier,
below.

**There is no demo administrator either.** The system runs one administrator,
and it is a real account, so its password is not in this file. The tests that
act as one read `TEST_ADMIN_EMAIL` and `TEST_ADMIN_PASSWORD` from the
...
It needs a real database and it is slow, because every query is a round trip to
Supabase in Mumbai. It is deliberately **not** in CI: those tests sign in as
real accounts and write to a real database, and putting that connection string
into a public repository's secrets is not a trade worth making.

Of its 410 tests, **101 run against an empty database** — they register,
verify, promote and delete everybody they touch. The other **309 do not create
the people they sign in as**: they assume a population with a history of
appointments, records and readings already there. That population is gone, so
those 309 skip unless `TEST_DEMO_DATA=1` says you have one of your own, with the
emails and password the tests name at the top of each file.

Skipping is the honest answer. An empty database is not a broken endpoint, and
309 failures that all read `invalid credentials` would describe the fixtures
rather than the code under test.

```bash
TEST_ADMIN_EMAIL=you@example.org TEST_ADMIN_PASSWORD=... TEST_DEMO_DATA=1 \
  .venv/Scripts/python.exe -m pytest tests/ -k integration
```
...
See **[DEPLOYMENT.md](DEPLOYMENT.md)**. The frontend belongs on Vercel and the
API is happier on a container host in Mumbai, but the whole thing fits on Vercel
if it has to — at the cost of one addition and one loss.

The addition is scheduling. A serverless function is frozen between requests, so
the reminder dispatcher's own loop never gets a second tick; `POST
/api/internal/dispatch` exists so an external scheduler can supply the ticks
instead. Overlapping calls are safe — the query claims rows with `FOR UPDATE
SKIP LOCKED` — and the endpoint refuses outright when `DISPATCH_SECRET` is
unset, because `compare_digest("", "")` is true and an unset secret that
compared equal would leave it open to anybody.

The loss is the live vitals stream, which needs a connection held open.

Sign-in used to be the other trap: session cookies are `SameSite=Lax` and carry
no `domain`, so a frontend and an API on different sites silently lost the
cookie after the login screen. The frontend now proxies `/api/*` to the API, so
they are one origin and the question no longer arises.
...
That is every test that does not need the database — 906 of them, in under half
a minute. The exclusion is the point: `*_integration.py` is the tier below.
...
```sql
REVOKE UPDATE, DELETE ON audit_logs FROM medinova_app;
```
...
not on the server. The audio never reaches MediNova AI, is never stored, and never
